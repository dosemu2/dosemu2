...
static int create_symlink_ex(const char *path, int number, int special,
	const char *path2)
{
	char *drives_c = assemble_path(dosemu_image_dir_path, "drives/c", 0);
...
static int install_dosemu_freedos (int choice)
{
	...
	if (choice == 3) {
		...
		boot_dir_path = assemble_path(dosemu_image_dir_path, "drive_c", 0);
...
static int first_boot_time(void)
{
	int first_time;
	char *dir_name =
		assemble_path(dosemu_image_dir_path, "drives", 0);
