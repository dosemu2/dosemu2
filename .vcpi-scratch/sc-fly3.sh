...
while [ $SECONDS -lt 1740 ]; do
  ...
  # the intro runs for many minutes; poke Escape and Return so it is skipped
  # and the game gets to the briefing and into the air
  if [ $clicked -gt 0 ] && [ $((SECONDS % 10)) -lt 2 ]; then
    /tmp/claude-0/xkey :7 Escape >/dev/null 2>&1
    sleep 1
    /tmp/claude-0/xkey :7 Return >/dev/null 2>&1
    cp -f $O/cur.xwd $O/k-$SECONDS.xwd 2>/dev/null
  fi
  sleep 2
done
