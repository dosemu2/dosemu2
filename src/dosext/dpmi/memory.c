...
{
  if (mprotect_mapping(MAPPING_DPMI | MAPPING_SHARED, DOSADDR_REL(ptr), size,
	PROT_READ | PROT_WRITE | PROT_EXEC) == -1)
    return 0;
  /* smalloc does memset to 0 */
  return 1;
}
...
static int SetAttribsForPage(unsigned int ptr, us attr, us *old_attr_p)
{
    ...
    if (change) {
      ...
      if (com) {
        ...
        if (mprotect_mapping(MAPPING_DPMI | MAPPING_SHARED,
            ptr, PAGE_SIZE, PROT_NONE) == -1) {
...
static void restore_page_protection(dpmi_pm_block *block)
{
  ...
  for (i = 0; i < block->size >> PAGE_SHIFT; i++) {
    if ((block->attrs[i] & 7) == 0) {
      mprotect_mapping(MAPPING_DPMI | MAPPING_SHARED,
