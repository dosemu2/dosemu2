#!/bin/bash
# MENU.EXE asks EMS for the page frame and hands its allocator 320K starting
# there; with the frame at 0xe000 that runs past the end of real mode memory.
# Give it twenty conventional windows instead and see whether it gets past the
# click.
exec > /tmp/scems20.log 2>&1
export DOSEMU2_COMCOM_DIR=/usr/local/share/comcom32
D=/home/user/dosemu2/bin/dosemu
O=/tmp/claude-0/scems20
MENU=cbdb02e32e1e
rm -rf $O; mkdir -p $O
cat > $O/rc <<'RC'
$_cpu_vm = "kvm"
...
while [ $SECONDS -lt 870 ]; do
  ...
  m=$(grab)
  if [ $SECONDS -gt 240 ] && [ "$m" = "$last" ] && [ -n "$m" ] && [ $clicked -lt 4 ]; then
