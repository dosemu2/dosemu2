...
for t in 220 260 300 340 380 420 460 500 560 620 680 740 800 860; do
  wait_to $((t-20))
  kill -0 $DP 2>/dev/null || { echo "dosemu gone before ${t}s"; break; }
  keys Return
  wait_to $((t-10))
...
echo "=== XFAULT ==="; grep -a 'XFAULT' /tmp/claude-0/scmenu2/sc.out /tmp/claude-0/scmenu2/sc.log 2>/dev/null
