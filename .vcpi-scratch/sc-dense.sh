...
printf 'SCCD.EXE\r\n' > PD.BAT
timeout 480 $D -s -X -f $O/rc -o $O/sc.log -D+E -K . -E "PD.BAT" </dev/null > $O/sc.out 2>&1 &
DP=$!
shot () {
  local n=$1 id
...
# dense sweep with no keys at all: does a menu ever flash between loops?
for t in $(seq 200 3 440); do
  wait_to $t
  kill -0 $DP 2>/dev/null || { echo "dosemu gone before ${t}s"; break; }
  shot $t
done
...
echo "=== XFAULT ==="; grep -a 'XFAULT' /tmp/claude-0/scdense/sc.out /tmp/claude-0/scmenu2/sc.log 2>/dev/null
