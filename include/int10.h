...
extern inline void poshgacur(int, int);
...
__inline__ void int10(u_char ii)
{
 ...
 v_printf("unallowed video mode 0x%x\n", LO(ax));
 CARRY;
 return;
      }
    case 7:
 ...
 if (config.usesX ) ; /* something to do */
      }
      else {
	...
	CARRY;
      ...
      if ( config.usesX )
        poshgacur(bios_cursor_x_position(bios_current_screen_page),
