#!/bin/bash
# Click PLAY, skip the intro with Escape, then watch where the game lands.
exec > /tmp/scfly4.log 2>&1
export DOSEMU2_COMCOM_DIR=/usr/local/share/comcom32
D=/home/user/dosemu2/bin/dosemu
O=/tmp/claude-0/scfly4
MENU=cbdb02e32e1e
rm -rf $O; mkdir -p $O
cat > $O/rc <<'RC'
$_cpu_vm = "kvm"
...
while [ $SECONDS -lt 1620 ]; do
  ...
  if [ $clicked -eq 1 ]; then
    d=$((SECONDS - t0))
    # skip the intro for the first three minutes after the click
    if [ $d -lt 200 ]; then
