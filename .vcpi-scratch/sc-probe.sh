...
printf 'SCCD.EXE\r\n' > PD.BAT
timeout 400 $D -s -X -f $O/rc -o $O/sc.log -D+DE -K . -E "PD.BAT" </dev/null > $O/sc.out 2>&1 &
DP=$!
shot () {
  local n=$1 id
...
echo "=== XFAULT ==="; grep -a 'XFAULT' /tmp/claude-0/scprobe/sc.out /tmp/claude-0/scmenu2/sc.log 2>/dev/null
