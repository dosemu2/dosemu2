...
printf 'SCCD.EXE\r\n' > PD.BAT
timeout 620 $D -s -X -f $O/rc -o $O/sc.log -D+E -K . -E "PD.BAT" </dev/null > $O/sc.out 2>&1 &
DP=$!
shot () {
  local n=$1 id
...
for t in 220 260 300 340 380 420 460 500 560; do
  wait_to $((t-25))
  kill -0 $DP 2>/dev/null || { echo "dosemu gone before ${t}s"; break; }
...
echo "=== XFAULT ==="; grep -a 'XFAULT' /tmp/claude-0/scctl/sc.out /tmp/claude-0/scmenu2/sc.log 2>/dev/null
