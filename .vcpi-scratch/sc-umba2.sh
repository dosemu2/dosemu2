...
while [ $SECONDS -lt 870 ]; do
  ...
  m=$(grab)
  if { [ "$m" = "$MENU" ] || [ $SECONDS -ge 240 ]; } && [ $clicked -lt 1 ]; then
...
echo "=== which files the game opened (atime, reset to 2020 before the run):"
ls -lu --time-style=+%m-%d_%H:%M /tmp/claude-0/sc/*.EXE /tmp/claude-0/sc/*.OVL 2>/dev/null | awk '{print $6, $7}'
