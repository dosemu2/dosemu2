 ...
 *
 * June 11, 1996 ( 2.0.0-HACKER_TOOL )
 ...
 *
 * November 11, 1996 ( 2.1.8-HACKER_TOOL )
 * - Adapted to modules-2.1.8
 * - corrected the bss-ZERO error along a patch that Jacques Gelinas posted on
 *   on linux-kernel.
 *
 * ---------------------------
 *
...
#include "insmod.h"
#if defined(HACKER_TOOL) && (defined(WITHIN_DOSEMU) || defined(STAND_ALONE))
#define MODULES_VERSION "2.0.0"
#else
#include "../Version.h"
#endif
...
*/
int kernel_getversion ()
{
	int ret = uname (&uts_info);
	int v[3];
	v[0] = v[1] = v[2] = 0;
	kernelv = 0;
	if (ret != -1){
		char *pt;
		v[0] = atoi(uts_info.release);
		pt = strchr(uts_info.release,'.');
		if (pt != NULL){
			pt++;
			v[1] = atoi(pt);
			pt = strchr(pt,'.');
			if (pt != NULL){
				v[2] = atoi(pt+1);
				kernelv = v[0]*1000000+v[1]*1000+v[2];
			}
		}
	}
	return ret;
...
#else
/* do syscall create_module directly to avoid bug in libc
   Hans Lermen <lermen@elserv.ffm.fgan.de> */
static _syscall2(int, create_module, const char *, name, unsigned long, size)
static int our_create_module(const char *name, unsigned long size)
{
	/* Why all this fuss ?
		In linux 2.1, the address returned by create module
		point in kernel space which is now mapped at the top
		of user space. Above 2Gig. This looks like a negative
		number for a long. The normal syscall macro of linux 2.0
		(and all libc compile with linux 2.0 or below) consider
		that the return value is a negative number and consider
		it is an error number (A kernel convention, return value
		are positive or negative, indicating the error number).
		
		By checking the value of errno, we know if we have been
		fooled by the syscall2 macro and we fix it.
	*/
	int ret =  create_module(name,size);
	if (errno > 125 && ret == -1) {
		ret = -errno;
		errno = 0;
	}
...
#define create_module our_create_module  /* this fakes our kludge */
#endif

/*
	Normally, these structure are defined in module.h
	To provide a utility which work and compiled on any
	system (2.0.x or 2.1.x and higher), we have to forget
	about system header and duplicate the structure here.
...
#ifndef MODULE_2_1_7_SIG
	#define MODULE_2_1_7_SIG	((void*)0x00000217)
#endif


struct mod_routines_2_1_7 {
	void *signature;
	int (*init)(void);		/* initialization routine */
	void (*cleanup)(void);		/* cleanup routine */
	struct _einfo exceptinfo;
};

struct mod_routines_2_0_0 {
	int (*init)(void);		/* initialization routine */
	void (*cleanup)(void);		/* cleanup routine */
};

union mod_routines_union{
	struct mod_routines_2_1_7 mod21;
	struct mod_routines_2_0_0 mod20;
};

static int init_module(const char *name, void *code, unsigned codesize,
		union mod_routines_union *routines,
...
struct strpatch {
	int where;	/* offset from start of the string pointer */
	int what;	/* Offset of the string location */
	char *string;	/* offset in argv[i] */
...
static void push_string(char *string, int patch_offset)
{
	...
	(*(stringpatches + n_stringpatches)).string = string;
	(*(stringpatches + n_stringpatches)).len = len;
	++n_stringpatches;
}
	...
	/* Change values according to the parameters to "insmod" */
	{
		...
		while (argc > 1) {
			...
			*val = '\0'; /* mark end of tag */
			...
			change = findsym(param, NULL, tabcomp);
			if (change == NULL) {
				insmod_error ("Symbol '%s' not found", param);
				exit(2);
			}
			patchoff = symvalue(change);
...
#include <gdbm.h>
					...
					if ((dbf = gdbm_open(kd_dbmfile, 0,
						...
						if (data.dptr) {
							/* found a value! */
							memcpy((int *)(textseg + patchoff), data.dptr, data.dsize);
							free (data.dptr);
							/* ignore given info */
							break;
						}
				...
				if (*val < '0' || '9' < *val) {
					...
					if (*val != ',')
					{
				     		push_string(val, patchoff);
						patchoff += sizeof(char *);
					}
				}
				else { /* numerical */
					if (val[0] == '0') {
	...
	/* Now, we are going to copy the strings */
	if(n_stringpatches > 0)
	  {
	    int		sum = 0;
	    int		n = n_stringpatches;
	    u_int	string_offset = progsize;

	    /* Calculate the increase needed */
	    for(patch = stringpatches; n-- > 0; ++patch)
	      sum += patch->len + 1;

	    progsize += sum;
	    /* JEJB: don't like this, but if we're copying strings, better
	     * make sure memory actually exists to copy them into */
	    ...
	    for(patch = stringpatches; n-- > 0; ++patch)
	      {
		strncpy(textseg + string_offset, patch->string, patch->len);
		*(textseg + string_offset + patch->len) = '\0';
		patch->what = string_offset;
		string_offset += patch->len + 1;
	      }
	  }
	...
	routines.mod21.init = (int (*)(void)) init_func;
	routines.mod21.cleanup = (void (*)(void)) cleanup_func;
	{
		char *start=NULL, *end=NULL;
		if (!aout_flag){
			elf_sect_info ("__ex_table",addr,&start,&end);
			#if 0
				fprintf (stderr,"Locate exception %p %p %08lx\n",start,end,addr);
			#endif
		}
		routines.mod21.signature = MODULE_2_1_7_SIG;
		routines.mod21.exceptinfo.start = start;
		routines.mod21.exceptinfo.end = end;
		if (kernelv < 2001008){
			/* See kernel/module.c for an explanation (jg) */
			routines.mod20.init = (int (*)(void)) init_func;
			routines.mod20.cleanup = (void (*)(void)) cleanup_func;
		}
	}
...
int ksyms(int argc, char **argv)
{
	...
	for (ksym = ksymtab, i = 0 ; i < nksyms ; ++i, ksym++) {
		...
		(llseek(kmem, (off_t)ksym->value, SEEK_SET) != -1) &&
#endif
