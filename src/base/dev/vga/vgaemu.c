 ...
 *
 * For an excellent reference to programming SVGA cards see Finn Th�gersen's
...
static int vga_get_mem_base_offset(dosaddr_t addr)
{
  int i;
  for(i = 0; i < VGAEMU_MAX_MAPPINGS; i++) {
    dosaddr_t base = vga.mem.map[i].base_page * HOST_PAGE_SIZE;
    dosaddr_t end = base + (vga.mem.map[i].pages * HOST_PAGE_SIZE);
...
{
  unsigned vga_page;
  for (vga_page = vga_addr / HOST_PAGE_SIZE;
       vga_page <= (vga_addr + len - 1) / HOST_PAGE_SIZE; vga_page++)
...
void vga_write(dosaddr_t addr, unsigned char val)
{
  if (!vga.inst_emu || !vga_bank_access(addr)) {
    int offs = vga_get_mem_base_offset(addr);
    if (offs != -1) {
      vga.mem.base[offs] = val;
      vga_mark_dirty(offs, 1);
    } else {
      WRITE_BYTE(addr, val);
    }
    return;
...
void vga_write_word(dosaddr_t addr, unsigned short val)
{
  if (!vga.inst_emu || !vga_bank_access(addr)) {
    int offs = vga_get_mem_base_offset(addr);
    if (offs != -1) {
      UNIX_WRITE_WORD(&vga.mem.base[offs], val);
      vga_mark_dirty(offs, 2);
    } else {
      WRITE_WORD(addr, val);
    }
    return;
...
void vga_write_dword(dosaddr_t addr, unsigned val)
{
  if (!vga.inst_emu || !vga_bank_access(addr)) {
    int offs = vga_get_mem_base_offset(addr);
    if (offs != -1) {
      UNIX_WRITE_DWORD(&vga.mem.base[offs], val);
      vga_mark_dirty(offs, 4);
    } else {
...
void memcpy_to_vga(dosaddr_t dst, const void *src, size_t len)
{
  int i;
  if (!vga.inst_emu) {
    int offs = vga_get_mem_base_offset(dst);
    if (offs != -1) {
      memcpy(&vga.mem.base[offs], src, len);
...
void memcpy_dos_to_vga(dosaddr_t dst, dosaddr_t src, size_t len)
{
  ...
  if (!vga.inst_emu) {
    int offs = vga_get_mem_base_offset(dst);
    if (offs != -1) {
      MEMCPY_2UNIX(&vga.mem.base[offs], src, len);
      vga_mark_dirty(offs, len);
    } else {
...
void vga_memcpy(dosaddr_t dst, dosaddr_t src, size_t len)
{
  int i;
  if (!vga.inst_emu) {
    int offs_d = vga_get_mem_base_offset(dst);
    if (offs_d != -1) {
      int offs_s = vga_get_mem_base_offset(src);
      assert(offs_s != -1);
      memmove(&vga.mem.base[offs_d], &vga.mem.base[offs_s], len);
      vga_mark_dirty(offs_d, len);
    } else {
      MEMCPY_DOS2DOS(dst, src, len);
    }
    return;
...
void vga_memset(dosaddr_t dst, unsigned char val, size_t len)
{
  ...
  if (!vga.inst_emu) {
    int offs = vga_get_mem_base_offset(dst);
    if (offs != -1) {
      memset(&vga.mem.base[offs], val, len);
      vga_mark_dirty(offs, len);
    } else {
...
int vga_emu_protect_page(unsigned page, int prot, int instremu)
{
  int i;
  int sys_prot;
  ...
  if(
    ...
    if (instremu)
      ...
      i = mprotect_vga(VGAEMU_MAP_LFB_MODE, p, 1 * HOST_PAGE_SIZE, sys_prot);
  }
  else {
    if (instremu)
    ...
    int instremu)
{
  ...
  for (i = 0; i < VGAEMU_MAX_MAPPINGS; i++) {
    ...
    if (vga.mem.map[i].pages) {
      ...
      if(j >= 0 && j < vga.mem.map[i].pages) {
        /* the bank is not mapped over a window the client holds, see
         * vga_emu_map(); protecting that page would fault on the client's
         * own stores and have vgaemu take them for drawing */
...
static void _vga_kvm_sync_dirty_map(unsigned mapping)
{
  ...
  base = mapping == VGAEMU_MAP_LFB_MODE ? VGAEMU_PHYS_LFB_BASE :
    vga.mem.map[mapping].base_page * HOST_PAGE_SIZE;
  if (base == 0)
    return;
...
static int vgaemu_unmap(unsigned page)
{
  ...
  i = alias_mapping(MAPPING_VGAEMU,
    page * HOST_PAGE_SIZE, 1 * HOST_PAGE_SIZE,
    VGA_EMU_RW_PROT, LOWMEM(page * HOST_PAGE_SIZE))
  );
...
int vga_emu_pre_init(void)
{
  ...
  vga.mem.plane_pages = 0x10 * PAGE_SIZE / HOST_PAGE_SIZE;	/* 16 pages = 64k */
  vga.dac.bits = 6;
  vga.inst_emu = 0;
...
static int vga_emu_post_init(void)
{
  ...
  vga_msg(
    ...
    vga.mem.lfb_base_page * HOST_PAGE_SIZE,
    ...
    unsigned display_end, int pos)
{
  ...
  if(i == end_page + 1) {
    ...
    for (; i < vga.mem.pages; i++) {
      if (vga.mem.dirty_map[i]) {
        ...
        _vga_emu_adjust_protection(i, 0, DEF_PROT, 0, 0);
      }
    }
#endif
    return -1;
  ...
  for(j = i; j <= end_page && vga.mem.dirty_map[j]; j++) {
    ...
    if (j == pos && pos == (display_start / HOST_PAGE_SIZE) &&
	(display_start & (HOST_PAGE_SIZE - 1)) && vga.mem.dirty_map[j] == 1)
  ...
  veut->update_start = i * HOST_PAGE_SIZE;
  veut->update_len = (j - i) * HOST_PAGE_SIZE;
  max_len = display_end - veut->update_start;
  if (veut->update_len > max_len) {
    assert(veut->update_len - max_len < HOST_PAGE_SIZE);
...
static int __vga_emu_setmode(vga_mode_info *vmi, int width, int height)
{
  ...
  vga.mem.bank = 0;
  vga.mem.bank_pages = (vmi->buffer_len << 10) / HOST_PAGE_SIZE;
  ...
  vga.mem.map[VGAEMU_MAP_BANK_MODE].base_page = (vmi->buffer_start << 4) /
      HOST_PAGE_SIZE;
  vgaemu_map_bank();
...
/* The JEMM page frame and video memory share the same addresses, and
 * vga_emu_map() leaves the windows the client holds alone.  So when such a
 * window stops holding a page, the bank is missing there and has to be put
 * back, or the client would be talking to low memory instead of the screen.
 */
...
int vgaemu_map_bank(void)
{
  ...
  vga.mem.bank_base = vga.mem.map[VGAEMU_MAP_BANK_MODE].base_page * HOST_PAGE_SIZE;
  vga.mem.bank_len = vga.mem.map[VGAEMU_MAP_BANK_MODE].pages * HOST_PAGE_SIZE;
