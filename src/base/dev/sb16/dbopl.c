...
static inline void Operator__UpdateRelease(Operator *self, const Chip* chip ) {
	...
	if ( rate ) {
		...
		self->rateZero &= ~(1 << O_RELEASE);
		if ( !(self->reg20 & MASK_SUSTAIN ) ) {
			self->rateZero &= ~( 1 << O_SUSTAIN );
		}	
	} else {
		self->rateZero |= (1 << O_RELEASE);
		self->releaseAdd = 0;
...
static Bits Operator__TemplateVolume(Operator *self, OperatorState yes) {
	...
	case O_ATTACK:
		...
		if ( vol < ENV_MIN ) {
			self->volume = ENV_MIN;
			self->rateIndex = 0;
			Operator__SetState( self, O_DECAY );
			return ENV_MIN;
		}
		break;
	case O_DECAY:
		...
		if ( GCC_UNLIKELY(vol >= self->sustainLevel) ) {
			...
			if ( GCC_UNLIKELY(vol >= ENV_MAX) ) {
				self->volume = ENV_MAX;
				Operator__SetState( self, O_OFF );
				return ENV_MAX;
			}
			//Continue as sustain
			self->rateIndex = 0;
			Operator__SetState( self, O_SUSTAIN );
		}
		break;
	case O_SUSTAIN:
		if ( self->reg20 & MASK_SUSTAIN ) {
...
TEMPLATE_VOLUME(O_OFF)
TEMPLATE_VOLUME(O_RELEASE)
TEMPLATE_VOLUME(O_SUSTAIN)
TEMPLATE_VOLUME(O_ATTACK)
TEMPLATE_VOLUME(O_DECAY)

static const VolumeHandler VolumeHandlerTable[5] = {
        &Operator__TemplateVolumeO_OFF,
        &Operator__TemplateVolumeO_RELEASE,
        &Operator__TemplateVolumeO_SUSTAIN,
        &Operator__TemplateVolumeO_DECAY,
        &Operator__TemplateVolumeO_ATTACK,
};
...
#endif
		self->rateIndex = 0;
		Operator__SetState( self, O_ATTACK );
	}
	self->keyOn |= mask;
}
...
static void Operator__KeyOff(Operator *self, Bit8u mask ) {
	...
	if ( !self->keyOn ) {
		if ( self->state != O_OFF ) {
			Operator__SetState( self, O_RELEASE );
		}
	}
}
