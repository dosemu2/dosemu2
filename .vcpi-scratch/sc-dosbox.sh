#!/bin/bash
exec > /tmp/scdbx.log 2>&1
O=/tmp/claude-0/scdbx
rm -rf $O; mkdir -p $O
rm -f /tmp/.X8-lock
...
timeout 420 dosbox -conf /tmp/claude-0/dosbox.conf -noconsole > $O/dosbox.out 2>&1 &
DP=$!
grab () {
  local id
...
while [ $SECONDS -lt 400 ]; do
  ...
  if [ $SECONDS -ge 60 ] && [ $clicked -lt 1 ]; then
