...
static void ndis_build_tables(void)
{
    ...
    snprintf((char *)res->cc.CcName, NAME_LEN, "%s", NDIS_MODULE_NAME);
...
/*
 * Called by ndis.sys, which has PROTMAN$ open by then. Builds all the
 * tables and prepares the request block for the RegisterModule call
 * that ndis.sys issues next. The driver has no configuration of its
 * own, so PROTOCOL.INI is of no interest to it: it is the protocol
 * manager that reads it, to find out what to bind to what.
 */
static void ndis_setup(dosaddr_t rb, uint16_t seg)
{
    ndis_seg = seg;
    ...
    pd_printf("NDIS: module '%s' at %#x:%#x, MAC %02x:%02x:%02x:%02x:%02x:%02x\n",
	    NDIS_MODULE_NAME, ndis_seg, (unsigned)offsetof(struct ndis_res, cc),
	    my_mac[0], my_mac[1], my_mac[2], my_mac[3], my_mac[4], my_mac[5]);
}

static int ndis_open_link(void);
...
void ndis_helper(struct vm86_regs *regs)
{
	...
	if (ndis_open_link() < 0) {
	    LO_BYTE_d(regs->ebx) = NDIS_ERROR_NONET;
	    CARRY;
	    break;
	}
	...
	ndis_setup(SEGOFF2LINEAR(regs->es, LO_WORD(regs->ebx)),
		LO_WORD(regs->edx));
	break;
...
/*
 * The link is taken only when ndis.sys actually registers, not at
 * startup: an enabled but unused NDIS driver should not hold a link
 * that the other users of it - the packet driver, the NE2000 - may
 * want, as most of the back-ends provide only one.
 */
static int ndis_open_link(void)
{
    if (ndis_fd != -1)
