...
static void SDL_sync_shiftstate(Boolean make, SDL_Keycode kc, SDL_Keymod e_state)
{
	...
	if (!!(shiftstate & MODIFIER_SHIFT) != !!(e_state & SDL_KMOD_SHIFT)) {
		shiftstate ^= MODIFIER_SHIFT;
	}
	if (!!(shiftstate & MODIFIER_CTRL) != !!(e_state & SDL_KMOD_CTRL)) {
		shiftstate ^= MODIFIER_CTRL;
	}
	if (!!(shiftstate & MODIFIER_ALT) != !!(e_state & SDL_KMOD_LALT)) {
		shiftstate ^= MODIFIER_ALT;
	}
	if (!!(shiftstate & MODIFIER_ALTGR) != !!(e_state & (SDL_KMOD_RALT|SDL_KMOD_MODE))) {
		shiftstate ^= MODIFIER_ALTGR;
	}

	if (!!(shiftstate & MODIFIER_CAPS) != !!(e_state & SDL_KMOD_CAPS)
		&& (make || (kc != SDLK_CAPSLOCK))) {
		shiftstate ^= MODIFIER_CAPS;
	}
	if (!!(shiftstate & MODIFIER_NUM) != !!(e_state & SDL_KMOD_NUM)
		...
		SDL_TextInputEvent textevent)
{
	...
	SDL_Scancode scan = keyevent.scancode;
	...
	src_len = strlen(p);
	rc = charset_to_unicode_string(&state, _key, &p, src_len, 2);
	cleanup_charset_state(&state);
	assert(rc == 1);

	assert(keyevent.down == true);
	SDL_sync_shiftstate(1, key, mod);
	/* don't trust unicode string with Alt modifier */
	if ((mod & (SDL_KMOD_LALT | SDL_KMOD_RALT)) || !config.layout_auto)
		move_keynum_grpsym(1, keynum, _key[0]);
	else
		move_keynum(1, keynum, _key[0]);
}
...
void SDL_process_key_release(SDL_KeyboardEvent keyevent)
{
	...
	assert(keyevent.down == false);
	SDL_sync_shiftstate(0, key, mod);
