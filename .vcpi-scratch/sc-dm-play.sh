...
while [ $SECONDS -lt 870 ]; do
  ...
  m=$(grab)
  if { [ "$m" = "$MENU" ] || [ $SECONDS -ge 330 ]; } && [ $clicked -lt 1 ]; then
    ...
    for k in $(seq 1 40); do
      sleep 5
      echo "  after+$((k*5))s de0c=$(grep -ac 'VCPI: switch to PM' $O/sc.log) md5=$(grab)"
      cp -f $O/cur.xwd $O/after-$base-$k.xwd
      # cinematics are skipped with Esc or space; the briefing wants Return
      case $((k % 3)) in
        0) /tmp/claude-0/xkey :7 Escape >/dev/null 2>&1 ;;
        1) /tmp/claude-0/xkey :7 space >/dev/null 2>&1 ;;
        2) /tmp/claude-0/xkey :7 Return >/dev/null 2>&1 ;;
      esac
    done
  fi
