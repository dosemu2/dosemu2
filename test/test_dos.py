...
from subprocess import (call, check_call, DEVNULL, STDOUT, CalledProcessError,
                        TimeoutExpired)
...
#        RW|C C C  N N N  N N N  N N N  Y Y Y
    ...
    def _test_ds3_share_open_delren_run_all(self, fstype, tests, testtype):
        testdir = "test-imagedir/dXXXXs/d"
...
""")
    ...
    def _test_ds3_share_open_delren_check_single_result(self, results, t):
        m = re.search("FAIL:\('%s', '%s', '%s'\)\[.*\]" % t, results)
        if m:
            self.fail(msg=m.group(0))

    def _test_ds3_share_open_delren(self, fstype, testtype):
        results = self._test_ds3_share_open_delren_run_all(fstype, self.DELRENTESTS, testtype)
        for t in self.DELRENTESTS:
            with self.subTest(t=t):
