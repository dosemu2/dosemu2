...
printf 'SCCD.EXE\r\n' > PD.BAT
timeout 330 $D -s -X -f $O/rc -o $O/sc.log -D+E -K . -E "PD.BAT" </dev/null > $O/sc.out 2>&1 &
DP=$!
shot () {
  local n=$1 id
  id=root
  [ -n "$id" ] && xwd -root -silent > $O/win-$n.xwd 2>/dev/null
