...
static int sdl_load_font(const char *name)
{
  ...
  sdl_fdesc[num_fdescs].rw = SDL_IOFromFile(pth, "r");
  FcPatternDestroy(match);
  FcPatternDestroy(pat);
...
static int SDL_init(void)
{
...
#else
  window = SDL_CreateWindow(config.X_title, 0, 0, flags);
  if (!window) {
    error("SDL window failed: %s\n", SDL_GetError());
    goto err;
  }
  renderer = SDL_CreateRenderer(window, rflags);
  if (!renderer) {
  ...
  SDL_GetMasksForPixelFormat(pixel_format, &bpp, &rm, &gm, &bm, &am);
  SDL_csd.bits = bpp;
  SDL_csd.r_mask = rm;
  SDL_csd.g_mask = gm;
  SDL_csd.b_mask = bm;
...
static void do_redraw(void)
{
  ...
  if (!surface) {
#if defined(HAVE_SDL_TTF) && defined(HAVE_FONTCONFIG)
    SDL_RenderTexture(renderer, texture_ttf, NULL, NULL);
#endif
  } else {
    SDL_RenderTexture(renderer, texture_buf, NULL, NULL);
  }
  pthread_mutex_unlock(&tex_mtx);
...
static TTF_Font *do_open_font(int idx, int psize, int *w, int *h)
{
  ...
  SDL_SeekIO(sdl_fdesc[idx].rw, 0, SDL_IO_SEEK_SET);

  f = TTF_OpenFontIO(sdl_fdesc[idx].rw, 0, psize);
  if (!f) {
    error("TTF_OpenFontRW: %s\n", SDL_GetError());
    return NULL;
  }

  if (!TTF_FontIsFixedWidth(f)) {
    TTF_CloseFont(f);
  ...
  // get metrics
  *h = TTF_GetFontLineSkip(f);
  TTF_GetGlyphMetrics(f, 'W', &minx, &maxx, &miny, &maxy, w);
  return f;
}
...
static void setup_ttf_winsize(int xtarget, int ytarget)
{
  ...
  if (old_tex) {
    SDL_FRect old = { .w = real_win_width, .h = real_win_height };
    SDL_FRect ne = { .w = r_xtarget, .h = r_ytarget };
    SDL_FRect un;
    SDL_GetRectIntersectionFloat(&old, &ne, &un);
    pthread_mutex_lock(&tex_mtx);
    SDL_SetRenderTarget(renderer, texture_ttf);
    SDL_SetRenderDrawColor(renderer, 0, 0, 0, 0);
    SDL_RenderClear(renderer);
    SDL_RenderTexture(renderer, old_tex, &un, &un);
    SDL_SetRenderTarget(renderer, NULL);
    pthread_mutex_unlock(&tex_mtx);
...
int SDL_set_videomode(struct vid_mode_params vmp)
{
  ...
  mode = SDL_GetCurrentDisplayMode(SDL_GetDisplays(NULL)[0]);
  if (!mode) {
    error("get current mode failed: %s\n", SDL_GetError());
    return 0;
  }
...
static void SDL_change_mode(int x_res, int y_res, int w_x_res, int w_y_res)
{
  ...
  if (!initialized) {
    ...
    if (config.X_fullscreen) {
      ...
      dm = SDL_GetCurrentDisplayMode(SDL_GetDisplays(NULL)[0]);
      w_x_res = dm->w;
      w_y_res = dm->h;
    }
...
static void window_size_changed(int w, int h)
{
	...
	SDL_GetRenderLogicalPresentation(renderer, &m_x_res, &m_y_res, NULL);
	if (!m_x_res || !m_y_res) {
	  m_x_res = w;
	  m_y_res = h;
	}
...
static void SDL_handle_events(void)
{
  ...
  while (SDL_PeepEvents(&event, 1, SDL_GETEVENT, SDL_EVENT_FIRST,
	...
	do
	  rc = SDL_PeepEvents(&key_event, 1, SDL_GETEVENT, SDL_EVENT_KEY_DOWN,
		SDL_EVENT_KEY_DOWN);
	...
	if ((mod & SDL_KMOD_CTRL) && (mod & SDL_KMOD_ALT)) {
	  if (key == mgrab_key || key == SDLK_K) {
	    force_grab = 0;
	    toggle_grab(key == SDLK_K);
	    break;
	  } else if (key == SDLK_F) {
	    toggle_fullscreen_mode();
	    ...
	    wait_kup = 1;
	    break;
	  } else if (key == SDLK_B && !config.X_fullscreen) {
	    border_on ^= 1;
    ...
    case SDL_EVENT_KEY_UP: {
      SDL_Keycode key = event.key.key;
      wait_kup = 0;
      if (copypaste && (key == SDLK_LSHIFT || key == SDLK_RSHIFT)) {
        copypaste = 0;
        if (!m_cursor_visible)
	    SDL_HideCursor();
      }
	...
	mouse_move_buttons(!!(buttons & SDL_BUTTON_MASK(1)),
			   !!(buttons & SDL_BUTTON_MASK(2)),
			   !!(buttons & SDL_BUTTON_MASK(3)),
			   MOUSE_SDL);
	break;
      }
...
//  v_printf("SDL_draw_string\n");
  ...
  SDL_Surface *srf = TTF_RenderText_Shaded(sdl_font, s, 0,
    ...
    Bit8u attr)
{
  ...
  d.tex = SDL_CreateSurface(font_width * len, 1, pixel_format);
  assert(d.tex);
  ...
  SDL_RenderLine(rend, 0, 0, font_width * len - 1, 0);
                               ...
                               int start, int end, Boolean focus)
{
  ...
  d.tex = SDL_CreateSurface(rect.w, rect.h, pixel_format);
  assert(d.tex);
  rend = SDL_CreateSoftwareRenderer(d.tex);
  SDL_SetRenderDrawColor(rend,
                           text_colors[ATTR_FG(attr)].r,
