...
static void draw_cursor(void)
{
  ...
  if (check_cursor_location
    ...
    if (cs > ce) {
      if (cs < vga.char_height - 1) {
        for (i = 0; i < num_texts; i++) {
          if (!(Text[i]->flags & TEXTF_DISABLED))
            Text[i]->Draw_cursor(Text[i]->opaque, x, y, XATTR(cursor, x, y),
		      cs, vga.char_height - 1, have_focus);
        }
      }
      cs = 0;
    }
...
static void calculate_selection(void)
{
  ...
  start = location_to_memoffs(sel_start_row * vga.scan_len +
			    sel_start_col * 2);
  end = location_to_memoffs(sel_end_row * vga.scan_len +
			    sel_end_col * 2);
