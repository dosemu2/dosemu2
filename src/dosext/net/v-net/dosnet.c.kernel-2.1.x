 ...
 */
unsigned short int 
dosnet_eth_type_trans(struct sk_buff *skb, struct device *dev)
{
	int result;
...
static int
dosnet_close(struct device *dev)
{
#ifdef MODULE
	MOD_DEC_USE_COUNT;
#endif    
...
static void
dosnet_set_multicast_list( struct device *dev )
{
   /* do nothing */
}
...
init_module(void)
{
	dev_dosnet = (struct device *)kmalloc(sizeof(struct device), 
					      GFP_KERNEL);
