...
/* function codes, as of revision 1.11 of the packet driver spec.
 * "+" marks the high-performance functions, "*" the extended ones. */
...
/* values returned in AL by driver_info() */
#define L_BASIC		1
#define L_EXTENDED	2
#define L_HIGH_PERF	5
#define L_HP_EXTENDED	6
...
/* The 1.11 spec names these two for the functions it added, but its
 * appendix of error codes was never extended past CANT_RESET, so the
 * numbers below are just the obvious continuation of the list. */
#define E_BAD_SIGNAL	16
...
#define RCV_RAW		7	/* serial line drivers only */

/* structure types for get_structure() */
