...
printf 'SCCD.EXE\r\n' > PD.BAT
timeout 420 $D -s -X -f $O/rc -o $O/sc.log -D+E -K . -E "PD.BAT" </dev/null > $O/sc.out 2>&1 &
DP=$!
shot () {
  local n=$1 id
...
echo "=== XIO ==="; grep -a "^ERROR: XIO" $O/sc.out | head -40
echo "=== XKB ==="; grep -a "^XKB" $O/sc.out | tail -3
echo "=== XFAULT ==="; grep -a 'XFAULT' /tmp/claude-0/scio/sc.out /tmp/claude-0/scmenu2/sc.log 2>/dev/null
