...
relocate_elf(FILE *fp, long offset)
{
	...
	if (bss_size > 0) memset(secref[bss_seg], 0, bss_size);
...
/*
	Locate the start address and end address of a section
	Return -1 if the section does not exist
*/
int elf_sect_info (
	const char *name,
	...
	char **end)
{
	...
	*start = *end = NULL;
	for(i=0; i < header.e_shnum; ++i){
	        if (strcmp((char*)sections[i].sh_name,name)==0){
			char *addr = (char*)(secref[i] - textseg);
			addr += loadaddr;
			*start = addr;
			*end   = addr + sections[i].sh_size;
			#if 0
			{
				int o;
				char **_addr = (char**)(secref[i]);
				for (o=0; o<sections[i].sh_size; o+= 8){
