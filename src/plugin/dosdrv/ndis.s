 ...
 *
 * Load it _after_ protman.dos, from userhook.sys, as dosemu supplies
 * its own config.sys, e.g.
 *   DEVICE=C:\NET\PROTMAN.DOS /I:C:\NET
 *   DEVICE=D:\DOSEMU\NDIS.SYS
 * and add a [DE2NDIS] section with DriverName=DE2NDIS$ to PROTOCOL.INI.
 *
 * Author: Stas Sergeev
 */
...
Init:
	...
	/* let dosemu build its tables and the registration request */
	movb	$DOS_HELPER_NDIS_HELPER, %al
...
NoNetMsg:
	.ascii	"ERROR: dosemu has no network link for NDIS, see its log.\r\n$"
NoProtmanMsg:
