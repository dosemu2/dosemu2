...
class EMUTestCase(ppdosgit(OurTestCase, {
    ...
    @mark('cputest')
    def test_cpu_fpu_undef_jit(self):
        """CPU undefined FPU opcodes (JIT)"""
        cpu_fpu_undef(self, 0)
    ...
    def test_cpu_fcmov_jit(self):
        """CPU FCMOVcc and F(U)COMI(P) (jit)"""
        cpu_fcmov(self, 0)

    @mark('cputest')
    def test_cpu_fcmov_sim(self):
        """CPU FCMOVcc and F(U)COMI(P) (sim)"""
        cpu_fcmov(self, 1)
