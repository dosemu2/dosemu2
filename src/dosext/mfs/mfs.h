...
#define _sdb_template_name(sdb)	((char     *)&sdb[sdb_template_name_off])
#define _sdb_template_ext(sdb)	((char     *)&sdb[sdb_template_ext_off])
#define	_sdb_attribute(sdb)	(*(u_char  *)&sdb[sdb_attribute_off])
#define _sdb_dir_entry(sdb)	(*(u_short *)&sdb[sdb_dir_entry_off])
#define _sdb_p_cluster(sdb)	(*(u_short *)&sdb[sdb_p_cluster_off])
#define	_sdb_file_name(sdb)	((char     *)&sdb[sdb_file_name_off])
#define	_sdb_file_ext(sdb)	((char     *)&sdb[sdb_file_ext_off])
#define	_sdb_file_attr(sdb)	(*(u_char  *)&sdb[sdb_file_attr_off])
#define	_sdb_file_time(sdb)	(*(u_short *)&sdb[sdb_file_time_off])
#define	_sdb_file_date(sdb)	(*(u_short *)&sdb[sdb_file_date_off])
#define _sdb_file_st_cluster(sdb)(*(u_short *)&sdb[sdb_file_st_cluster_off])
#define _sdb_file_size(sdb)	(*(u_int   *)&sdb[sdb_file_size_off])
#define _sft_handle_cnt(sft) 	(*(u_short *)&sft[sft_handle_cnt_off])
#define _sft_open_mode(sft)  	(*(u_short *)&sft[sft_open_mode_off])
#define _sft_attribute_byte(sft) (*(u_char  *)&sft[sft_attribute_byte_off])
#define _sft_device_info(sft)  	(*(u_short *)&sft[sft_device_info_off])
#define	_sft_dev_drive_ptr(sft)	(*(u_int   *)&sft[sft_dev_drive_ptr_off])
#define	_sft_start_cluster(sft)	(*(u_short *)&sft[sft_start_cluster_off])
#define	_sft_time(sft)		(*(u_short *)&sft[sft_time_off])
#define	_sft_date(sft)		(*(u_short *)&sft[sft_date_off])
#define	_sft_size(sft)		(*(u_int   *)&sft[sft_size_off])
#define	_sft_position(sft)	(*(u_int   *)&sft[sft_position_off])
#define _sft_rel_cluster(sft)	(*(u_short *)&sft[sft_rel_cluster_off])
#define _sft_abs_cluster(sft)	(*(u_short *)&sft[sft_abs_cluster_off])
#define	_sft_directory_sector(sft) (*(u_short *)&sft[sft_directory_sector_off])
#define	_sft_directory_entry(sft)  (*(u_char  *)&sft[sft_directory_entry_off])
#define	_sft_name(sft)		( (char    *)&sft[sft_name_off])
#define	_sft_ext(sft)		( (char    *)&sft[sft_ext_off])
#define	_sft_fd(sft)		(*(u_short *)&sft[sft_fd_off])
#define	_sda_current_dta(sda)	(FARADDR((far_t *)&sda[sda_current_dta_off]))
#define	_sda_error_code(sda)		(*(u_short *)&sda[4])
#define _sda_cur_psp(sda)		(*(u_short *)&sda[sda_cur_psp_off])
#define __sda_cur_drive(sda)		(*(u_char *)&sda[sda_cur_drive_off])
#define _sda_filename1(sda)		((char  *)&sda[sda_filename1_off])
#define	_sda_filename2(sda)		((char  *)&sda[sda_filename2_off])
#define _sda_sdb(sda)			((sdb_t    )&sda[sda_sdb_off])
#define	_sda_cds(sda)		((cds_t)(uintptr_t)(FARPTR((far_t *)&sda[sda_cds_off])))
#define _sda_search_attribute(sda)	(*(u_char *)&sda[sda_search_attribute_off])
#define _sda_open_mode(sda)		(*(u_char *)&sda[sda_open_mode_off])
#define _sda_rename_source(sda)		((sdb_t    )&sda[sda_rename_source_off])
#define _sda_user_stack(sda)		((char *)(uintptr_t)(FARPTR((far_t *)&sda[sda_user_stack_off])))
#define _sda_ext_act(sda)		(*(u_short *)&sda[sda_ext_act_off])
#define _sda_ext_attr(sda)		(*(u_short *)&sda[sda_ext_attr_off])
#define _sda_ext_mode(sda)		(*(u_short *)&sda[sda_ext_mode_off])
#define _psp_parent_psp(psp)		(*(u_short *)&psp[0x16])
#define _psp_handles(psp)		((char *)(uintptr_t)(FARPTR((far_t *)&psp[0x34])))
#define _lol_dpbfarptr(lol)		(rFAR_FARt(READ_DWORD((lol)+lol_dpbfarptr_off)))
#define _lol_cdsfarptr(lol)		(rFAR_FARt(READ_DWORD((lol)+lol_cdsfarptr_off)))
#define _lol_last_drive(lol)		(READ_BYTE((lol)+lol_last_drive_off))
#define _lol_nuldev(lol)		        ((lol)+lol_nuldev_off)
#define _lol_njoined(lol)		((lol)+lol_njoined_off)
...
extern int dos_unlink_lfn(const char *fpath, int drive);
