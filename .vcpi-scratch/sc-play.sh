...
printf 'SCCD.EXE\r\n' > PD.BAT
timeout 400 $D -s -X -f $O/rc -o $O/sc.log -D+E -K . -E "PD.BAT" </dev/null > $O/sc.out 2>&1 &
DP=$!
shot () {
  local n=$1 id
...
for t in 30 50 70 90 110 140 170 200 240 280 320 360; do
