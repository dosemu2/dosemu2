...
/*
 * The name under which the driver registers itself with the protocol
 * manager. It is the name by which the protocols refer to the adapter,
 * so the PROTOCOL.INI section describing this adapter has to be named
 * like that too.
 */
#define NDIS_MODULE_NAME "DE2NDIS"

/*
 * Name of the DOS character device implementing the MAC driver.
 * It has to be matched by the DriverName= keyword of that section.
 */
#define NDIS_DEVICE_NAME NDIS_MODULE_NAME "$"

/* sub-functions of DOS_HELPER_NDIS_HELPER, see ndis.sys */
#define NDIS_SUBHELPER_INIT      0  /* query availability and memory size */
#define NDIS_SUBHELPER_CONFIG    1  /* build tables, prepare registration */
...
#define NDIS_ERROR_NONET         3  /* no networking available */
